...
export function CommentSection({ postId, tableName = 'post_comments', likesTableName = 'post_comment_likes' }: CommentSectionProps) {
    const { comments, loading, createComment, updateComment, deleteComment, toggleCommentLike, userLikedComments } = useComments(postId, tableName, likesTableName);
    const { user } = useAuth();
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (success) {
            ...
            toast.error(t('common.error', { defaultMessage: 'Erro ao enviar.' }));
        }
    };
    ...
    return (
        <div className="mt-16 pt-12 border-t border-app">
            <h3 className="text-2xl font-bold font-display mb-8 flex items-center gap-3">
                <MessageCircle className="w-6 h-6 text-[#D70F24]" />
                {t('community.answerPage.respostas', { defaultMessage: 'Comentários' })} ({comments.length})
            </h3>
